...
export default defineMcpTool({
  ...
  async handler({ query }) {
    try {
      ...
      if (isTokenQuery) {
        const token21Content = await $fetch<string>(`${baseUrl}/llms-design-tokens-21.txt`, {
          responseType: 'text'
        })
        const token24Content = await $fetch<string>(`${baseUrl}/llms-design-tokens-24.txt`, {
          responseType: 'text'
        })

        relevantContent.push({
          title: 'Design Tokens v2.1 vs v2.4 Comparison',
          content: `# Design Tokens Comparison\n\n## Token 2.1\n${token21Content.substring(
            0,
            1500
          )}\n\n## Token 2.4\n${token24Content.substring(
            0,
            1500
