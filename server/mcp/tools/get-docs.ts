...
export default defineMcpTool({
  ...
  inputSchema: {
    query: z.string().describe('Your question or search query (e.g., "how to setup Pixel", "difference between token 2.1 and 2.4", "MpButton", "dark mode")')
  },
  async handler({ query }) {
    try {
      ...
      if (relevantContent.length === 0) {
        return jsonResult({
          query: query,
          found: false,
          message: 'No specific documentation found for your query. Here are some suggestions:',
          suggestions: [
      ...
      return errorResult(`Error reading documentation: ${error instanceof Error ? error.message : 'Unknown error'}`)
    }
  }
})
